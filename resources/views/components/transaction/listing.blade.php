<div>
    <!-- It is not the man who has too little, but the man who craves more, that is poor. - Seneca -->
        ...
        <div class="mb-5 mt-10">
            ...
            <div class="text-muted font-weight-bold font-size-sm">Your latest transactions</div>
        </div>
        <!--begin::Title-->
        @foreach ($transactions as $transaction)
            ...
            <a href="{{ route('transaction', [
                ...
                <div class="d-flex align-items-center mb-9 {{ $transaction->type === 'debit' ? 'bg-light-danger' : 'bg-light-success' }} rounded p-5">
                    <!--begin::Icon-->
                    <span class="svg-icon {{ $transaction->type === 'debit' ? 'svg-icon-danger' : 'svg-icon-success' }} mr-5">
                    ...
                    <div class="d-flex flex-column flex-grow-1 mr-2">
                        <span href="#" class="font-weight-bold text-dark-75 text-hover-primary font-size-lg mb-1">{{ $transaction->title }}</span>
                        <span class="text-muted font-weight-bold">{{ $transaction->remarks }}</span>
                    </div>
                    <!--end::Title-->
                    <!--begin::Lable-->
                    <span class="font-weight-bolder {{ $transaction->type === 'debit' ? 'text-danger' : 'text-success' }} py-1 font-size-lg">{{ $transaction->type === 'debit' ? '-' : '+' }}{{ $transaction->amount }}</span>
                    <!--end::Lable-->
                </div>
            </a>
            <!--end::Item-->
